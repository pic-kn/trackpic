...
function drawDefaultGreyPlaceholder(ctx, x, y, size) {
  ...
  // 「タップして写真を選択」テキスト (筑紫A丸ゴシック: CANVAS_FONT)
  ctx.fillStyle = "rgba(44, 50, 54, 0.55)";
...
export function renderMusicPlayerCanvas(ctx, options) {
  ...
  const maxTitleW = contentWidth - 90 * scale;
  let displayTitle = title || "Untitled";
  if (getTitleWidth(displayTitle) > maxTitleW) {
    while (displayTitle.length > 0 && getTitleWidth(displayTitle + "...") > maxTitleW) {
  ...
  const maxArtistW = contentWidth - 40 * scale;
  if (ctx.measureText(displayArtist).width > maxArtistW) {
    while (displayArtist.length > 0 && ctx.measureText(displayArtist + "...").width > maxArtistW) {
