...
export function extractPalette(image, focusX, focusY) {
  ...
  // 5色に満たない場合のスマートトーン補填 (単色写真でも綺麗な階調パレットを生成)
  const result = [...picked];
  if (result.length > 0) {
    const base = result[0];
    ...
    while (result.length < 5) {
      ...
      const targetL = Math.max(0.1, Math.min(0.9, l + (idx * 0.16 - 0.32)));
      const rgb = hslToRgb(h, Math.max(0.1, s), targetL);
      result.push({
        r: rgb.r,
        g: rgb.g,
        b: rgb.b,
